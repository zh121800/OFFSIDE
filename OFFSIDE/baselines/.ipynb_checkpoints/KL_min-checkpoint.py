...
def parse_args():
    ...
    parser.add_argument("--forget_data", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/forget_set.json")
    parser.add_argument("--retain_data", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/retain_set.json")
    parser.add_argument("--forget_batch_size", type=int, default=1)
    parser.add_argument("--retain_batch_size", type=int, default=3)
    parser.add_argument("--model_path", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/output/Qwen2.5-VL-LoRA-vanilla-2100")
    parser.add_argument("--target_model_path", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/output/Qwen2.5-VL-LoRA-vanilla-2100")
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/output/Qwen2.5-VL-LoRA-kl")
...
def process_func(example, tokenizer, processor, max_length, data_dir: str):
...
def main():
    ...
    forget_data_dir = os.path.dirname(os.path.abspath(args.forget_data))
    retain_data_dir = os.path.dirname(os.path.abspath(args.retain_data))
    processed_forget = forget_dataset.map(
        lambda ex: process_func(ex, tokenizer, processor, args.max_length, forget_data_dir),
        remove_columns=forget_dataset.column_names
    )
    processed_retain = retain_dataset.map(
        lambda ex: process_func(ex, tokenizer, processor, args.max_length, retain_data_dir),
