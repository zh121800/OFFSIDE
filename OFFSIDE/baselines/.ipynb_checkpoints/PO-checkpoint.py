...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/output/Qwen2.5-VL-LoRA-po")
...
def process_func(example, tokenizer, processor, max_length, data_dir: str, idk_text=None, is_forget=False):
...
def main():
    ...
    def preprocess_retain(examples):
        return process_func(examples, tokenizer, processor, args.max_length, retain_data_dir, idk_text=None, is_forget=False)
    # Forget set processing function, all labels replaced with idk_text
    def preprocess_forget(examples):
        return process_func(examples, tokenizer, processor, args.max_length, forget_data_dir, idk_text=args.idk_text, is_forget=True)
