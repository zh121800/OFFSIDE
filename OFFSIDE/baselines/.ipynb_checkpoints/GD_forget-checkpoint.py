...
def parse_args():
    ...
    # Data related
    parser.add_argument("--forget_data", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/forget_set_part.json", 
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/OFFSIDE/OFFSIDE/output_part/Qwen2.5-VL-LoRA-GD", 
...
def process_func(example, tokenizer, processor, max_length, data_dir: str):
    ...
    file_path = _resolve_image_path(example.get("images"), data_dir)  # image path
...
def main():
    ...
    forget_dataset = forget_ds.map(
        lambda example: process_func(example, tokenizer, processor, args.max_length, forget_data_dir),
    ...
    retain_dataset = retain_ds.map(
        lambda example: process_func(example, tokenizer, processor, args.max_length, retain_data_dir),
