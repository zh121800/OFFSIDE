...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen2.5-VL模型评估脚本（仅使用GPT进行评分）")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"GPT评分错误: {e}. {wait_time}秒后重试...")
                time.sleep(wait_time)
            else:
                print(f"GPT评分失败，已达到最大重试次数: {e}")
                return {"factuality_score": 0, "justification": f"评分失败: {str(e)}"}
...
def evaluate_prediction(pred_item, gpt_model):
    ...
    try:
        ...
        pred_item['gpt_justification'] = gpt_result.get('justification', "未提供")
        return pred_item
        
    except Exception as e:
        print(f"评估样本 {pred_item['example_id']} 时出错: {e}")
        pred_item['gpt_factuality_score'] = 0
        pred_item['gpt_justification'] = f"评估失败: {str(e)}"
        return pred_item
...
def main():
    ...
    print(f"第一阶段：并行批量推理 (batch_size={batch_size}, workers={inference_workers})...")
    

    pbar = tqdm(total=len(dataset), desc="模型推理进度", disable=args.quiet)
    ...
    print(f"模型推理阶段完成，共成功处理 {len(inference_results)}/{len(dataset)} 个样本")
    
    if not inference_results:
        print("没有成功的推理结果，程序终止")
        return
    
    print(f"第二阶段：并行API评分 (workers={args.api_workers})...")
    final_results = []

    pbar = tqdm(total=len(inference_results), desc="API评分进度", disable=args.quiet)
    ...
    if final_results:
        ...
        print(f"GPT事实性评分: {avg_gpt:.2f}/10")
        print(f"评估样本总数: {len(final_results)}")
    else:
        print("未生成任何评估结果。")
